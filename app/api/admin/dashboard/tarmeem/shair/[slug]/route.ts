// app/api/admin/dashboard/tarmeem/shair/[slug]/route.ts
...
export async function PATCH(
  ...
  try {
    ...
    const existingShair = await ShairModel.findOne({ slug });
    if (!existingShair) {
      return NextResponse.json(
        {
          success: false,
    ...
    const oldCoverImageUrl: string | null = existingShair.coverImage || null;
    ...
    // Update shair
    const updatedShair = await ShairModel.findOneAndUpdate(
      { slug },
      { ...updateData },
      {
        new: true,
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Shair updated successfully (تارمیم ہوگیا)",
        data: {
          shair: updatedShair,
          image: {
            oldImageDeleted,
            newImageUploaded: !!newCoverImageUrl,
            newImageUrl: newCoverImageUrl || updatedShair.coverImage,
          },
    ...
    console.error("Update Shair Error:", error);
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Failed to update shair",
        data: null,
        err: "UPDATE_ERROR",
        status: HTTP_STATUS.INTERNAL_SERVER_ERROR,
      },
