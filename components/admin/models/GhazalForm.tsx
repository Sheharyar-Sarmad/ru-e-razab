// components/admin/models/GhazalForm.tsx
"use client";
...
export default function GhazalForm({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || "نیٹ ورک کی خرابی";
      toast.error(msg, {
        style: {
          background: "#4A2B2B",
          color: "#FFF3EF",
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="نئی غزل تخلیق کریں">
              ...
              {fields.map((field, index) => (
                ...
                >
                  ...
                  <div className="mt-2 grid grid-cols-1 gap-3">
                    <input
                      {...register(`content.${index}.lines.0`)}
                    ...
                    />
                    <input
                      {...register(`content.${index}.lines.1`)}
                      className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:outline-none font-urdu"
                      style={{
                        borderColor: `${COLORS.deepForest}40`,
                        background: `${COLORS.warmWhite}40`,
