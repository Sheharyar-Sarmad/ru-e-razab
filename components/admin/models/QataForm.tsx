// components/admin/jadeed-kalam/QataForm.tsx
"use client";
...
const schema = z.object({
  ...
  content: z
    ...
    .length(2, "قطعہ میں بالکل 2 اشعار ہونے چاہئیں"),
...
interface QataFormProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function QataForm({ isOpen, onClose }: QataFormProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await axios.post("/api/admin/dashboard/jadeed/qata", formData, {
      ...
      if (response.data.success) {
        toast.success("قطعہ تخلیق ہوگیا! 🎉", {
          style: {
            background: "#2B4735",
            color: "#FFF3EF",
          },
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="نیا قطعہ تخلیق کریں">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" dir="rtl">
          ...
          <div>
            <label className="block text-sm font-medium" style={{ color: COLORS.deepForest }}>
              اشعار <span className="text-red-500">*</span> (بالکل 2)
            </label>
            <div className="mt-2 space-y-4">
              {[0, 1].map((shairIndex) => (
                <div key={shairIndex} className="p-4 rounded-lg border" style={{ borderColor: `${COLORS.deepForest}20` }}>
                  <span className="text-sm font-medium" style={{ color: COLORS.deepForest }}>
                    شعر #{shairIndex + 1}
                  </span>
                  <div className="mt-2 grid grid-cols-1 gap-3">
                    <input
                      {...register(`content.${shairIndex}.lines.0`)}
                    ...
                    />
                    <input
                      {...register(`content.${shairIndex}.lines.1`)}
                      className="w-full px-4 py-2 rounded-lg border focus:ring-2 focus:outline-none font-urdu"
                      style={{
                        borderColor: `${COLORS.deepForest}40`,
                        background: `${COLORS.warmWhite}40`,
                  ...
                  {errors.content?.[shairIndex]?.lines && (
                    <p className="mt-1 text-sm text-red-500">
                      {typeof errors.content[shairIndex].lines === 'object' && 'message' in errors.content[shairIndex].lines
                        ? errors.content[shairIndex].lines.message
                        : 'درست کریں'}
                    </p>
                  )}
                </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {linkFields.length < 5 && (
                <button
                  ...
                  style={{
                    background: `${COLORS.richMustard}20`,
