// components/admin/layout/ShairsClient.tsx
"use client";
...
import UpdateShairForm from "../models/UpdateShairForm";
import {
  PencilSquareIcon,
  TrashIcon,
  MagnifyingGlassIcon,
...
export default function ShairsClient({
  initialShairs,
  initialPage,
  totalPages,
  totalCount,
  ...
  const [shairs, setShairs] = useState(initialShairs);
  const [totalPagesState, setTotalPagesState] = useState(totalPages);
  const [totalCountState, setTotalCountState] = useState(totalCount);
  const [error, setError] = useState(initialError);

  const [selectedShair, setSelectedShair] = useState<Shair | null>(null);
      ...
      {error ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {shairs.map((shair, idx) => {
              const firstLine = getFirstLine(shair.content);
              const categories = getCategories(shair.category);
                ...
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto pt-3 flex items-center justify-end gap-2 border-t border-gray-100">
                      ...
                      <button
                        onClick={() => handleDelete(shair.slug)}
